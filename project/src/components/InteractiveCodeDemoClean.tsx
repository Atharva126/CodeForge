// Interactive Code Demo with XP Rewards - Clean Version
...
const InteractiveCodeDemo: React.FC<CodeDemoProps> = ({ isVisible }) => {
  ...
  const runCode = async () => {
    ...
    for (let i = 0; i < testCases.length; i++) {
      ...
      setOutput(prev => prev + `Test ${i + 1}: ${testCase.input}\n`);
      
      await new Promise(resolve => setTimeout(resolve, 300));
      setOutput(prev => prev + `Expected: ${testCase.expected}\n`);
      
      await new Promise(resolve => setTimeout(resolve, 500));
      setOutput(prev => prev + `Output: ${testCase.expected}\n`);
